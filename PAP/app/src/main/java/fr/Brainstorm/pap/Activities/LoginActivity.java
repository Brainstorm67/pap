...
/*        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    private void caseOK(final String email) {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (email.equals("test@test")){ // compte d'ajout on lance uniquement l'interface admin
                    final Intent logged = new Intent(LoginActivity.this,AdminActivity.class); // login OK on passe à CHeck
                    ...
                    final Intent logged = new Intent(LoginActivity.this,Main.class); // login OK on passe à CHeck
                    logged.putExtra("USER_EXTRA", regMilitant); // on envoie a Check les données utilisateur
                    LoginActivity.this.startActivityForResult(logged,USER_LOG_OUT);
                }
            }
